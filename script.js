...
var name;
    if (twitch[i].stream === null){
      name =(twitch[i].display_name)
      status = "Offline"
      logo = "http://errorlogz.com/wp-content/uploads/2015/08/no-error-300x300.jpg"
  } 
    else {
   name = (twitch[i].stream.display_name)
      status = (twitch[i].stream.status)
      logo = (twitch[i].stream.logo)
